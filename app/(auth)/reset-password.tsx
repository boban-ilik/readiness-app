...
export default function ResetPasswordScreen() {
  const { resetPasswordWithCode, sendPasswordResetCode } = useAuth();
  ...
  async function handleReset() {
    ...
    if (!email) {
      Alert.alert('Missing email', 'Go back and request a new code.');
      return;
    }
    if (token.length < 6) {
  ...
  async function handleResend() {
    ...
    try {
      ...
      Alert.alert('Code sent', 'Check your inbox for a fresh reset code.');
    } catch (error: any) {
